            ...
            <nav className="flex justify-between items-center h-fit mb-[2rem]">
                ...
                <div className="hidden lg:flex items-center justify-between w-[65%]">
                    ...
                    <a href="#Register">
                        ...
                        >
                            Register Now
                            <img src="/Images/header/arrow.svg" alt="Arrow up" className="w-[1rem] h-[1rem]" />
                        </motion.button>
                    </a>
                </div>

                {/* Mobile Hamburger Menu */}
                <div className="flex lg:hidden z-50 cursor-pointer" onClick={toggleMenu}>
                    <motion.div
                            ...
                            <ul className="flex flex-col items-center gap-[0.7rem] font-[500] text-[1.2rem] text-[#272F3A]">
                                {navItems.map((item) => (
                                    <li key={item.name} onClick={() => setIsMenuOpen(false)}>
                                        <a href={item.href} className="block py-[0.5rem] px-[1rem] hover:text-[#6B46C1] transition-colors duration-200">
                                            {item.name}
                                        </a>
                                    </li>
                                ))}
                            </ul>
                            <a href="#Register" onClick={() => setIsMenuOpen(false)} className="mt-[2rem]">
                                <motion.button
